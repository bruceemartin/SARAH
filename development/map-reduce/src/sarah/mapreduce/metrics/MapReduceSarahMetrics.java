...
public class MapReduceSarahMetrics extends SarahMetrics {
	...
	public MapReduceSarahMetrics(Configuration conf)  {
		...
		numberInputRecords = new CountedLongMapReduceMetric(numberInputRecords,conf);
		...
		numberSampleRecords = new CountedLongMapReduceMetric(numberSampleRecords,conf);
		sampleMetrics[6] = numberSampleRecords;
		
		numberOutputRecordsForF = new CountedLongMapReduceMetric(numberOutputRecordsForF,conf);
		numberOutputKeysForF = new CountedLongMapReduceMetric(numberOutputKeysForF,conf);
		minSizeRecordForF = new CountedLongMapReduceMetric(minSizeRecordForF,conf);
		maxSizeRecordForF = new CountedLongMapReduceMetric(maxSizeRecordForF,conf);
		totalSizeRecordsForF = new CountedLongMapReduceMetric(totalSizeRecordsForF,conf);
		meanSizeRecordForF = new CountedDoubleMapReduceMetric(meanSizeRecordForF,conf);
		percentile25ForF = new CountedDoubleMapReduceMetric(percentile25ForF,conf);
		percentile50ForF = new CountedDoubleMapReduceMetric(percentile50ForF,conf);
		percentile75ForF = new CountedDoubleMapReduceMetric(percentile75ForF,conf);
		...
		numberOutputRecordsForK = new CountedLongMapReduceMetric(numberOutputRecordsForK,conf);
		minSizeRecordForK = new CountedLongMapReduceMetric(minSizeRecordForK,conf);
		maxSizeRecordForK = new CountedLongMapReduceMetric(maxSizeRecordForK,conf);
		totalSizeRecordsForK = new CountedLongMapReduceMetric(totalSizeRecordsForK,conf);
		meanSizeRecordForK = new CountedDoubleMapReduceMetric(meanSizeRecordForK,conf);
		percentile25ForK = new CountedDoubleMapReduceMetric(percentile25ForK,conf);
		percentile50ForK = new CountedDoubleMapReduceMetric(percentile50ForK,conf);
		percentile75ForK = new CountedDoubleMapReduceMetric(percentile75ForK,conf);
